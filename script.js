...
function stopReading() {
  ...
  if (popup) { popup.innerHTML = ""; popup.classList.remove("show"); }
...
function presentCellBubble(cell, { images = [], videos = [], text = "" } = {}) {
  ...
  // BUGFIX: the container itself is invisible (opacity:0, pointer-events:none)
  // until it has the "show" class — without this the entire bubble system
  // (media + text) is built in the DOM but never actually visible on screen.
...
function removeBubbleGroup(group) {
  ...
  setTimeout(() => {
    ...
    // Only hide the container once every bubble in it is gone, so a
    // second overlapping bubble doesn't get hidden by the first one's
    // cleanup timer.
...
function playAndWaitForAudio(audio, overlapMs) {
  return new Promise((resolve) => {
    ...
    // BUGFIX: exposed so anything that yanks this audio element out from
    // under us (e.g. the max-concurrent-overlap watcher below) can run
    // the SAME cleanup as a natural finish — clearing tracking, removing
    // its bubble, and resolving this promise — instead of just pausing
    // the element and leaving a dangling bubble + a promise that would
    // otherwise sit unresolved for up to the full 120s safety timeout.
    audio._forceStop = function () {
...
(function () {
  ...
  setInterval(() => {
    ...
    while (list.length > MAX_CONCURRENT_AUDIO) {
      ...
      if (!oldest) break;
      // BUGFIX: use the same cleanup path a natural "ended" event would
      // (pause + remove element + clear tracking/bubble + resolve any
      // still-pending promise). The old version only paused/removed the
      // element, which left an orphaned bubble on screen forever and,
      // for any audio that hadn't already early-resolved, left the
      // reading loop's await hanging until the 2-minute safety timeout —
      // that's the freeze/"crash" during normal overlap use.
      if (typeof oldest._forceStop === "function") {
        oldest._forceStop();
      } else {
